...
When $C$ is small, the chain is very dependent on the past observations and as we can see on the last two plots, it hasn't visited the entire parametric space yet, so we would need much more samples in order to solve those two problems.
...
posterior_samples <- data.frame(
  ...
  sigma2_untruncated = sqrt(sample_posterior_untruncated$sigma2),
  sigma2_truncated = sqrt(post_sample_clean$sigma2)
)
...
#contours <- c(seq(0.05,0.95,0.1))
#this one is not normalized! but since we would divide by a constant we can still use the sape to see what is going on.
contour(
  x = mu_grid,
  y = log(sqrt(sigma2_grid)),
